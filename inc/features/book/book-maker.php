...
function rubik_admin_enqueues($hook)
{
	...
	wp_enqueue_style( 'html2pdf', THEME_URL . '/dist/css/html2pdf.css' );
}
	...
	if( is_object($item) )
	{
		...
		foreach ( $meta_data_items as $meta ) {
			...
			if( $meta->key == '_book_maker_pdf_content' && ! empty($is_workshop) ) {
				...
				echo '<a href="' . add_query_arg(['product-id' => $product_id, 'order-id' => $_GET['post'], 'item-id' => $item_id, 'font-size' => $font_size], THEME_URL . '/inc/features/book/generate-book-pdf.php') . '" class="button button-primary">Voir le PDF du livre</a>';
				echo '&nbsp;<a href="' . add_query_arg(['product-id' => $product_id, 'order-id' => $_GET['post'], 'item-id' => $item_id, 'regenerate' => 'true', 'font-size' => $font_size], THEME_URL . '/inc/features/book/generate-book-pdf.php') . '" class="button button-secondary">Regénérer le PDF</a>';
...
function book_maker_shortcodes( $atts )
{
	...
	if( $field_id ) {
		...
		if (
			...
			return TLTH::is_vowel($field_value) ? $atts['if_vowel'] : $atts['if_not_vowel'];
		}
